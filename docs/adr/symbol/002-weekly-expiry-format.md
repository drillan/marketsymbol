...
4. **derivatives-reaper での暗黙ルール**: 金曜は `YYYYMM-W`、水曜は `YYYYMMDD` と使い分けているが、これは文書化されていない暗黙の規則
...
- derivatives-reaper との関係：乖離の程度と理由を明確化
...
**デメリット**:
- derivatives-reaper の `YYYYMM` / `YYYYMM-W` 形式との乖離
- シンボル生成時にSQ日計算が必要
...
### derivatives-reaper との関係

| 項目 | derivatives-reaper | marketsymbol | 備考 |
|------|-------------------|--------------|------|
...
marketsymbol は derivatives-reaper の `YYYYMM` / `YYYYMM-W` 形式を**非推奨**とし、全て `YYYYMMDD` 形式に統一する。これは後方互換性を意図的に破棄する改善である。
...
derivatives-reaper 形式との相互変換が必要な場合：

```python
from datetime import date
from marketsched import get_sq_date  # 外部ライブラリから取得

def convert_deriv_reaper_expiry(expiry: str) -> str:
    """derivatives-reaper 限月形式を marketsymbol 形式に変換
...
def convert_to_deriv_reaper_monthly(expiry: str) -> str:
    """marketsymbol 月次限月を derivatives-reaper 形式に変換
...
- derivatives-reaper との互換性がない（`YYYYMM` / `YYYYMM-W` 形式）
...
- 変換関数を提供することで derivatives-reaper との相互運用は可能
